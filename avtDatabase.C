...
//
//    Makani Buckley, Tue Jun 9 13:40:00 PDT 2026
//    Don't skip !TIME and !ENSEMBLE keywords. Sort !KEYWORDS to the start of
//    the file list.
//
// ****************************************************************************
    ...
    char **&filelist, int &filelistN, int *bang_nBlocksp)
{
    ...
    vector<char *>  list;
    vector<char *> tmp_file_list; // temporary list of files
    char  str_auto[1024];
    ...
    while (!ifile.eof() && !failed)
    {
		...
		tmp_file_list.push_back(str_heap);
                ++fileCount;
            }
            ++goodCount;
    ...
    // insert files after keywords
    list.insert(list.end(), tmp_file_list.begin(), tmp_file_list.end());
